...
int main()
{
	...
	flow = initialMaxFlow;
	prevFlow = flow;
	cout << "Starting the K Static Attack on the Static Routing" << endl;
	for(int x = 30; x <= 60; x++)
	{
		...
		cout << "ROUNDS TO GET FLOW TO 0 WITH K = " << x << ": " << i << endl;
		SSK << i << " " << x << "\n";
		ResetAll(nodesPtr, nodeAmount, totalLink);		
	...
	flow = initialMaxFlow;
	prevFlow = flow;
	cout << "Starting the K Reactive Attack on the Static Routing" << endl;
	for(int x = 30; x <= 60; x++)
        {
        	flow = MaxFlow(S, D, nodesPtr, nodeAmount, totalLink);
        	i = 0;
	...
	flow = initialMaxFlow;
	prevFlow = flow;
	cout << "Randomly Attacking the K Static Routing" << endl;
	for(int x = 30; x <= 60; x++)
        {
		flow = MaxFlow(S, D, nodesPtr, nodeAmount, totalLink);
		i = 0;
...
void ResetKLinks(Node& S, Node& D, const int nodeAmount, Node *nodesPtr)
{
	...
	for (int i = 0; i < linkAmount; i++)//for each link in S
	{
		if ((linkPtr + i)->getBreak() == false) //if the link is not breakable
		{
			...
			(linkPtr + i)->setStoredCapacity(0);
			(linkPtr + i)->setCapacity(0);
			S.decrementLinks();
		}
	}
	for (int i = 0; i < nodeAmount; i++) //for each node
	{
		...
		if (linkAmount > 0) //if there are still links
		{
			...
			{
				if ((linkPtr + j)->getTarget() == D.getVertex() && (linkPtr + j)->getBreak() == false) //
				{
					(linkPtr + j)->setTarget(-1);
					(linkPtr + j)->setBreak(true);
					(linkPtr + j)->setStoredCapacity(0);
					(linkPtr + j)->setCapacity(0);
